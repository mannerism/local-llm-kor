...
이 시리즈는 **"한국 중학생도 따라올 수 있는 수준"** 을 목표로 합니다. PR 보내시기 전에 가이드 한 번만 읽어주세요. 두 가지 길:

- 손으로 직접 쓰실 분 → [HUMAN-GUIDE.md](./HUMAN-GUIDE.md)
- Claude / ChatGPT / Cursor 같은 LLM 에 넘기실 분 → [LLM-GUIDE.md](./LLM-GUIDE.md) (통째로 컨텍스트로 붙여넣고 "이 가이드 대로 작성해줘" 하시면 됨)
...
- **다른 편·primitive 언급은 위키링크**: `[[7-model-run/index|7편]]` 식. 평문 X
...
> 외부 기여자는 `.env.local` 설정 필요 없습니다 — vault 동기화는 메인테이너 워크플로라
> 자동으로 건너뛰고, `docs/guide/` 의 커밋된 콘텐츠 그대로 사이트가 뜹니다.
...
이 저장소 전체(코드·본문·이미지) 는 **[CC BY-SA 4.0](./LICENSE)** 으로 배포됩니다 — 자유롭게 복사·재배포·개작 가능. 단 (a) 저작자 표시, (b) 개작물도 동일 라이선스 공개 의무.
...
- [ ] 전문 용어에 비유 설명이 붙어 있나요?
- [ ] 결정 지점마다 "왜"가 적혀 있나요?
- [ ] 스크린샷에 한 줄 설명이 _위에_ 붙어 있나요? (아래가 아니라 위)
- [ ] `pnpm dev` 로 로컬에서 빌드 에러 없이 렌더되나요?
- [ ] CLA 에 동의하시는지 확인하셨나요? (PR 제출 = 동의)
...
CC BY-SA 4.0 외 별도 라이선스가 필요하시면 <dearmannerism@gmail.com> 으로 연락주세요.
