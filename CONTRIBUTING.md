...
- **결정마다 "왜"**: 도구나 옵션을 고를 때마다 *왜 이걸 골랐는지* 한 문단 추가
...
### 콘텐츠 원본은 Obsidian vault

이 저장소의 `docs/guide/` 는 별도 Obsidian vault에서 `pnpm sync` 로 동기화됩니다. 큰 콘텐츠 변경은 vault 쪽에서 진행 후 sync하는 워크플로지만, **저장소에 직접 PR을 보내셔도 머지 후 메인테이너가 vault에 반영합니다.** 외부 기여자는 GitHub 쪽만 신경 쓰시면 돼요.
...
- [ ] 전문 용어에 비유 설명이 붙어 있나?
- [ ] 결정 지점마다 "왜"가 적혀 있나?
- [ ] 스크린샷에 한 줄 설명이 *위에* 붙어 있나? (아래가 아니라 위)
- [ ] `pnpm sync && pnpm dev` 로 로컬에서 빌드 에러 없이 렌더되는가?
