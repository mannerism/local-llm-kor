...
## 이게 뭐예요?
**모델이 내 맥북에 들어갈 수 있느냐의 문제예요.** [[memory-bandwidth]]이 "얼마나 빠른가"를 결정한다면, 메모리 용량은 "돌릴 수 있는가 없는가"를 결정합니다.
...
- **[[kv-cache|KV 캐시]]**: 이전 입력을 저장해두는 공간. 컨텍스트가 길수록 커짐 (긴 글 분석할 땐 GB 단위로 추가)
- **macOS와 다른 앱**: macOS만 5~8GB는 먹어요. 브라우저·Slack 등 켜져 있으면 더 늘어남

## 헤드룸 — 60% 룰

**모델이 통합 메모리의 60% 이하를 차지하게 잡으세요.** 나머지 40%는 [[kv-cache|KV 캐시]]·다른 앱·OS 몫이에요.
...
2. **빠듯한 경우**: [[kv-cache|KV 캐시]]가 짧아지고 컨텍스트 길이 줄여야 함
