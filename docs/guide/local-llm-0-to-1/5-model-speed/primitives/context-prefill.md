...
같은 프롬프트를 여러 번 쓰면 첫 번째만 prefill 비용이 들고, 두 번째부터는 캐시에서 바로 가져와요. 이걸 **[[kv-cache|KV 캐시]]** (또는 prompt caching) 라고 합니다.
...
큰 컨텍스트는 [[kv-cache|KV 캐시]] 메모리도 더 먹으니까, 평소 작업 사이즈에 맞춰 잡으면 됩니다.

## 정리
- 입력이 길수록 prefill로 첫 응답이 늦어짐
- 8K 토큰 이상에선 prefill이 시간 대부분 차지
- 멀티턴 대화는 [[kv-cache|KV 캐시]] 덕에 두 번째 턴부터 빠름
- `-c` 플래그로 컨텍스트 크기 조절
