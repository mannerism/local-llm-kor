...
이 기본편에서는 **가장 단순한 모델 이름**을 읽는 법만 다뤄요. 더 복잡한 표기(MoE, 양자화, 파일 형식 등)는 응용편에서 봅니다.
...
> 💡 더 복잡한 이름 (예: `froggeric/Qwen3.6-27B-MTP-GGUF`)은 응용편에서 다룹니다.
...
### 이게 뭐예요?
기본편에서는 `Qwen3.6-27B-Instruct` 같은 단순한 이름을 봤어요. 근데 실제 Hugging Face에서 보면 이런 복잡한 이름이 더 많습니다:
...
*이 페이지의 원본은 Obsidian vault `Personal/LocalLLM/4-model-name/index.md` 에서 동기화됩니다.*
