...
> 빌드되는 동안에는 아래 [로컬에서 보기](#로컬에서-보기) 로 띄워서 보실 수 있어요.
...
```sh
pnpm install
pnpm dev      # 사이트가 http://localhost:5173 에 뜸
```

`pnpm dev` 가 알아서 vault → `docs/guide/` 동기화 후 VitePress dev 서버를 띄웁니다. 메인테이너가 아니라 단순히 사이트만 띄워 보고 싶다면 vault 동기화는 무시돼도 사이트는 정상 동작해요 (현재 `docs/guide/` 에 마지막 동기화 결과가 커밋돼 있음).

빌드 결과만 보고 싶다면:

```sh
pnpm build       # docs/.vitepress/dist/ 에 정적 파일 생성
pnpm preview     # 빌드 결과 미리보기 서버
```
